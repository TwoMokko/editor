...
class Editor {
    ...
    constructor() {
        ...
        /* Building DOM */
        $('body').prepend(this.top_container.append(this.btn_container.append(this.btn_rotate, this.btn_crop, this.btn_blur, this.btn_bright), this.toolbar.append(this.tool_rotate.append(this.clockwise, this.counter_clockwise, this.upside), this.tool_blur.append(this.brush1, this.brush2, this.brush3, this.brush4), this.tool_bright.append(this.touch), this.btn_reset), this.end_container.append(this.reset_all, this.save_img, this.exit)));
        /* Onload image */
        ...
        this.btn_rotate.on('click', () => { if (this.state == States.None)
            return; if (this.btn_rotate.hasClass('btn_use')) {
            this.state = States.Ready;
            this.UseBtn();
        }
        else
            this.state = States.Rotate; this.UseBtn(); this.ChangeToolbar(); });
        this.btn_crop.on('click', () => { if (this.state == States.None)
            return; if (this.btn_crop.hasClass('btn_use')) {
            this.state = States.Ready;
            this.UseBtn();
        }
        else
            this.state = States.Crop; this.UseBtn(); this.ChangeToolbar(); });
        this.btn_blur.on('click', () => { if (this.state == States.None)
            return; if (this.btn_blur.hasClass('btn_use')) {
            this.state = States.Ready;
            this.UseBtn();
        }
        else
